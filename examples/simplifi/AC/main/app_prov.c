...
struct app_prov_data {
    onboard_t *pc;                      /*!< Protocomm handler */
...
static esp_err_t app_prov_start_service(dev_info_t *pt_dev)
{
    /* Create new protocomm instance */
    g_prov->pc = uob_mob2dev_new();
    if (g_prov->pc == NULL) {
        ESP_LOGE(TAG, "Failed to create new protocomm instance");
        return ESP_FAIL;
    }

    pt_dev->pt_onb=g_prov->pc;
    ...
    if (uob_mob2dev_start(pt_dev) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to start tcp_serve task");
        return ESP_FAIL;
    }
                                             ...
                                             int security, const protocomm_security_pop_t *pop)
{
    ...
    /* Start provisioning service through tcp server */
