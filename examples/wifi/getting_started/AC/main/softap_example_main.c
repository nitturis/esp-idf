...
int collect_data(int sock, pbuff_t *pt_buff){
   			char *pbuf=NULL;
            switch(pt_buff->type){
                case INT:
                case SHORT:
...
#if 1
int  parse_mob2dev_aes( char *aes_buffer,dev_info_t *pt_dev){
...
int decrypt_mobile_ob_data(unsigned char *key, unsigned char *iv,unsigned char *payload,size_t payload_len, dev_info_t *pt_dev){
    ...
    if(dev_rsa_priv_key_int(&pt_dev->tsec.dev_priv_ctx,NULL)){
        ...
        ESP_LOGI(TAG,"decrypt_mobile_ob_data \n");
    ...
    parse_mob2dev_aes((char *)aes_ctx->plain_buffer,pt_dev);
    return 0;

}

int ob_mob2dev_parse(pbuff_t *pb,dev_info_t *pt_dev){
    ...
    if( !decrypt_mobile_ob_data((unsigned char *)key->valuestring,(unsigned char *)iv->valuestring,(unsigned char *)payload->valuestring,payload_len,pt_dev))
        goto end;
...
{
    size_t olen=0;
    //mbedtls_rsa_context *dev_rsa=NULL;

    int eret=0;
    //Initialize ESP_ERR_NVS_NEW_VERSION_FOUNDS
